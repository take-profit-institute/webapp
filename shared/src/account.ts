...
export const Transaction = Type.Object(
  {
    ...
    name: Type.String(),
    quantity: Type.Number(),
    price: Type.Number({ description: 'Execution price per share (체결가 우선, 미체결이면 주문가)' }),
    amount: Type.Number({ description: 'quantity * price (체결 금액)' }),
    fee: Type.Number(),
    // 체결 상세 — trading-service가 체결 시 채운다(미체결/mock은 생략).
    tax: Type.Optional(Type.Number({ description: '거래세 (매도 체결만)' })),
    netAmount: Type.Optional(Type.Number({ description: '정산금액 (수수료·세금 반영)' })),
    executedPrice: Type.Optional(Type.Number({ description: '실제 체결 단가 (미체결이면 생략)' })),
